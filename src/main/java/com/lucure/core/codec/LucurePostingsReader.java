...
public final class LucurePostingsReader extends PostingsReaderBase {
  ...
  /** Sole constructor. */
  public LucurePostingsReader(
    Directory dir, FieldInfos fieldInfos, SegmentInfo segmentInfo,
    ...
    try {
      docIn = dir.openInput(IndexFileNames.segmentFileName(segmentInfo.name, segmentSuffix, LucurePostingsFormat.DOC_EXTENSION),
                            ioContext);
      version = CodecUtil.checkHeader(docIn,
                            LucurePostingsWriter.DOC_CODEC,
                            LucurePostingsWriter.VERSION_START,
                            LucurePostingsWriter.VERSION_CURRENT);
      forUtil = new ForUtil(docIn);

      if (fieldInfos.hasProx()) {
        posIn = dir.openInput(IndexFileNames.segmentFileName(segmentInfo.name, segmentSuffix, LucurePostingsFormat.POS_EXTENSION),
                              ioContext);
        CodecUtil.checkHeader(posIn, LucurePostingsWriter.POS_CODEC, version, version);

        if (fieldInfos.hasPayloads() || fieldInfos.hasOffsets()) {
          payIn = dir.openInput(IndexFileNames.segmentFileName(segmentInfo.name, segmentSuffix, LucurePostingsFormat.PAY_EXTENSION),
                                ioContext);
          CodecUtil.checkHeader(payIn, LucurePostingsWriter.PAY_CODEC, version, version);
        }
      }
  ...
  public void init(IndexInput termsIn) throws IOException {
    ...
    CodecUtil.checkHeader(termsIn,
                          LucurePostingsWriter.TERMS_CODEC,
                          LucurePostingsWriter.VERSION_START,
                          LucurePostingsWriter.VERSION_CURRENT);
  ...
  public void decodeTerm(long[] longs, DataInput in, FieldInfo fieldInfo, BlockTermState _termState, boolean absolute)
    ...
    if (version < LucurePostingsWriter.VERSION_META_ARRAY) {  // backward compatibility
  ...
  public DocsEnum docs(FieldInfo fieldInfo, BlockTermState termState, Bits liveDocs, DocsEnum reuse, int flags) throws IOException {
    return docsAndPositions(fieldInfo, termState, liveDocs, null, flags | DocsAndPositionsEnum.FLAG_PAYLOADS);
  }
...
//      return everythingEnum.reset(liveDocs, (IntBlockTermState) termState, flags);
    ...
    public void checkIntegrity() throws IOException {
        if (version >= LucurePostingsWriter.VERSION_CHECKSUM) {
            if (docIn != null) {
    ...
    private LucureSkipReader skipper;
    ...
    public BlockDocsEnum(FieldInfo fieldInfo) throws IOException {
      this.startDocIn = LucurePostingsReader.this.docIn;
      this.docIn = null;
    ...
    public int advance(int target) throws IOException {
      ...
      if (docFreq > BLOCK_SIZE && target > nextSkipDoc) {
        ...
        if (skipper == null) {
          ...
          skipper = new LucureSkipReader(docIn.clone(),
                                        LucurePostingsWriter.maxSkipLevels,
                                        BLOCK_SIZE,
                                        indexHasPos,
                                        indexHasOffsets,
                                        indexHasPayloads);
  ...
  final class BlockDocsAndPositionsEnum extends DocsAndPositionsEnum {
    ...
    public BlockDocsAndPositionsEnum(FieldInfo fieldInfo) throws IOException {
      this.startDocIn = LucurePostingsReader.this.docIn;
      this.docIn = null;
      this.posIn = LucurePostingsReader.this.posIn.clone();
      encoded = new byte[MAX_ENCODED_SIZE];
      indexHasOffsets = fieldInfo.getIndexOptions().compareTo(IndexOptions.DOCS_AND_FREQS_AND_POSITIONS_AND_OFFSETS) >= 0;
      indexHasPayloads = fieldInfo.hasPayloads();
    }
  ...
  final class EverythingEnum extends DocsAndPositionsEnum {
    ...
    public EverythingEnum(FieldInfo fieldInfo) throws IOException {
      ...
      this.docIn = null;
      this.posIn = LucurePostingsReader.this.posIn.clone();
      this.payIn = LucurePostingsReader.this.payIn.clone();
      encoded = new byte[MAX_ENCODED_SIZE];
      indexHasOffsets = fieldInfo.getIndexOptions().compareTo(IndexOptions.DOCS_AND_FREQS_AND_POSITIONS_AND_OFFSETS) >= 0;
      if (indexHasOffsets) {
