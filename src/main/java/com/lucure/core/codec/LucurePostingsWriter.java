 ...
 *
 * @see LucureSkipWriter for details about skipping setting and postings layout.
 * @lucene.experimental
 */
public final class LucurePostingsWriter extends PostingsWriterBase {
  ...
  private final LucureSkipWriter skipWriter;
  
  /** Creates a postings writer with the specified PackedInts overhead ratio */
  // TODO: does this ctor even make sense?
  public LucurePostingsWriter(
    SegmentWriteState state, float acceptableOverheadRatio) throws IOException {
    super();

    docOut = state.directory.createOutput(IndexFileNames.segmentFileName(state.segmentInfo.name, state.segmentSuffix, LucurePostingsFormat.DOC_EXTENSION),
                                          state.context);
    ...
    try {
      ...
      if (state.fieldInfos.hasProx()) {
        ...
        posOut = state.directory.createOutput(IndexFileNames.segmentFileName(state.segmentInfo.name, state.segmentSuffix, LucurePostingsFormat.POS_EXTENSION),
                                              state.context);
        ...
        if (state.fieldInfos.hasPayloads() || state.fieldInfos.hasOffsets()) {
          payOut = state.directory.createOutput(IndexFileNames.segmentFileName(state.segmentInfo.name, state.segmentSuffix, LucurePostingsFormat.PAY_EXTENSION),
                                                state.context);
    ...
    skipWriter = new LucureSkipWriter(maxSkipLevels,
                                     BLOCK_SIZE,
